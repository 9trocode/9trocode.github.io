---
date: 2025-11-01
authors:
  - alex
categories:
  - Personal
  - Welcome
---

# Welcome to My Blog

Welcome to my personal blog. I write about cloud infrastructure, DevOps, and software engineering.

<!-- more -->

## What to Expect

On this blog, you'll find content about:

- **Cloud Infrastructure**: Best practices, tutorials, and real-world experiences
- **DevOps & Automation**: CI/CD pipelines, infrastructure as code, and automation strategies
- **Software Engineering**: Code quality, architecture patterns, and development tips
- **Open Source**: Projects I'm working on and contributing to
- **Career Growth**: Lessons learned and advice for fellow engineers

## A Bit About Me

I'm Alex Idowu, a Cloud Infrastructure and Software Engineer based in Lagos, Nigeria. Currently working at PipeOpsHQ, building scalable infrastructure solutions.

I focus on:

- Building reliable and scalable systems
- Automating repetitive tasks (because why do manually what you can automate?)
- Contributing to open source projects
- Sharing knowledge with the community

## Why This Blog?

I've always believed in the power of sharing knowledge. Throughout my career, I've learned so much from blog posts, documentation, and the generous sharing of others in the tech community. This blog is my way of giving back.

Whether you're a seasoned engineer or just starting out, I hope you'll find something useful here.

## Let's Connect

I'd love to hear from you! Feel free to reach out:

- GitHub: [@9trocode](https://github.com/9trocode)
- Twitter: [@nitrocode](https://twitter.com/nitrocode)
- LinkedIn: [Alex Idowu](https://www.linkedin.com/in/nitrocode/)

Stay tuned for more content, and happy reading!
